...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/copilot-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <model>] [--effort <none|low|medium|high|xhigh|max>] [prompt]",
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!copilotStatus.available) {
    nextSteps.push("Install the GitHub Copilot CLI (see https://docs.github.com/copilot/how-tos/copilot-cli).");
  }
  if (copilotStatus.available && !authStatus.loggedIn && authStatus.requiresAuth) {
...
function buildReviewPrompt(promptName, context, focusText) {
  const template = loadPromptTemplate(ROOT_DIR, promptName);
  return interpolateTemplate(template, {
    TARGET_LABEL: context.target.label,
    USER_FOCUS: focusText || "No extra focus provided.",
    REVIEW_COLLECTION_GUIDANCE: context.collectionGuidance,
    REVIEW_INPUT: context.content,
    OUTPUT_SCHEMA: readOutputSchema(REVIEW_SCHEMA_PATH)
  });
}
...
function ensureCopilotAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error(
      "The GitHub Copilot CLI is not installed or not on PATH. Install it, then rerun `/copilot:setup`."
    );
  }
}
...
async function executeReviewRun(request) {
  ...
  if (reviewName === "Review" && focusText) {
    throw new Error(
      "`/copilot:review` does not support custom focus text. Retry with `/copilot:adversarial-review " +
        focusText +
        "` for focused review instructions."
    );
  }

  const context = collectReviewContext(request.cwd, target);
  const promptName = reviewName === "Adversarial Review" ? "adversarial-review" : "review";
  const prompt = buildReviewPrompt(promptName, context, focusText);
...
function enqueueBackgroundTask(cwd, job, request) {
  ...
  // Write the job record (with its request payload) before spawning the
  // worker. The worker reads this file immediately on startup via
  // readStoredJob, so spawning first can race it to a "No stored job found"
  // failure if the child schedules before this write lands.
  const preSpawnRecord = {
    ...job,
    status: "queued",
    phase: "queued",
    pid: null,
  ...
  writeJobFile(job.workspaceRoot, job.id, preSpawnRecord);
  upsertJob(job.workspaceRoot, preSpawnRecord);

  const child = spawnDetachedTaskWorker(cwd, job.id);
  const queuedRecord = { ...preSpawnRecord, pid: child.pid ?? null };
...
async function handleCancel(argv) {
  ...
  const interrupt = await interruptCopilotTurn(cwd, { threadId, turnId });
