...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model>] [--effort <none|low|medium|high|xhigh|max>] [what Copilot should investigate, solve, or continue]"
...
`copilot:copilot-rescue` is a subagent, not a skill — do not call `Skill(copilot:copilot-rescue)` (no such skill).
...
- If the helper reports that Copilot CLI is missing or unauthenticated, stop and tell the user to run `/copilot:setup`.
