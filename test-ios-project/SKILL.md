...
4. **UI Test - XCUITest（iOS App のみ）**
   ...
   - **Swift Package Manager / ライブラリの場合**
     ...
     - スキップした理由を記録し、そのまま次の Mutation Test に進んでください。

5. **`muter run` - Muter Mutation Test**
   ...
   - **合格ライン: ミューテーションスコア 80% 以上**

## 設定と補足

- format → lint → unit（coverage 集計含む）→ UI → mutation の順に個別で実行し、前のステップに失敗がないことを確認してから次へ進んでください。
...
- カバレッジが 90% に届かない場合は、未カバーのブランチを特定し、境界値テストや条件分岐テストを追加してください。

## 期待される成果

- 実装後に5ステップを順に実行することで、iOS プロジェクトのフォーマット整形・静的解析・ユニットテスト・UI テスト・ミューテーションテストの信頼性が確保される。
- iOS App と Swift Package の違いを踏まえ、実行可能な UI テスト戦略だけを採用できる。
- 各ステップは成功するまで繰り返す習慣が生まれ、合格ライン（ブランチカバレッジ ≥ 90%、ミューテーションスコア ≥ 80%）を順番に満たしていく段階的な品質向上が可能となる。
