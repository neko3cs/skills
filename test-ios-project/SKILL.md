...
**User:** この画面の実装が終わったので品質ゲートを通してください。

**Agent:** `swiftformat .` → `swiftlint` → XCTest Unit Test（ブランチカバレッジ集計）→ XCUITest（iOS Appの場合）→ PBT → Muter の順に実行します。SwiftLintで未使用importの警告が出たので削除してから進めます。
