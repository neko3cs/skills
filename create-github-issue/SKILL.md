...
組み立てたタイトル・本文・ラベルをユーザーに提示し、内容に問題がないか確認を取る。Issue作成は他者に見える操作であり、作成後の取り消しは手間がかかるため、`gh issue create` の実行前に必ず確認すること。

### 6. `gh issue create` を実行する
...
**Agent:** バグ報告として作成します。最新のテンプレート定義を取得し、再現手順・期待する動作・実際の動作・バージョン・影響度・受け入れ条件などの必須項目を会話内容から埋めます。内容を提示しますので、作成前にご確認ください。
