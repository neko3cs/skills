...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/agy-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <model>] [prompt]",
...
async function buildSetupReport(cwd) {
  ...
  const authStatus = agyStatus.available ? await getAgyAuthStatus(cwd) : { loggedIn: false, detail: "Skipped: agy is not installed.", requiresAuth: true };

  const nextSteps = [];
  if (!agyStatus.available) {
    nextSteps.push("Install the Antigravity CLI (`agy`), e.g. via `winget install Google.AntigravityCLI`.");
  }
  if (agyStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Run `!agy` once interactively to complete sign-in, then rerun /agy:setup.");
  }

  return {
    ready: nodeStatus.available && agyStatus.available && authStatus.loggedIn,
    node: nodeStatus,
    agy: agyStatus,
    auth: authStatus,
...
function ensureAgyAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("The Antigravity CLI (`agy`) is not installed or not on PATH. Install it, then rerun `/agy:setup`.");
  }
}
...
function findLatestResumableTaskJob(jobs) {
  return (
    jobs.find((job) => job.jobClass === "task" && job.status !== "queued" && job.status !== "running") ?? null
  );
}
...
async function resolveLatestTrackedTaskThread(cwd, options = {}) {
  ...
  if (trackedTask) {
    // agy has no queryable thread id; resumption always means "continue the
    // most recent agy conversation on this machine" (see lib/agy.mjs).
    return { id: null };
  }
...
async function executeTaskRun(request) {
  ...
  if (request.resumeLast) {
    ...
    if (!latestThread) {
      throw new Error("No previous agy task was found for this repository's Claude session.");
    }
    resumeLast = true;
  }
  ...
  const result = await runAgyTurn(workspaceRoot, {
    ...
    defaultPrompt: resumeLast ? DEFAULT_CONTINUE_PROMPT : "",
    model: request.model,
...
function enqueueBackgroundTask(cwd, job, request) {
  ...
  // worker, since the worker reads this file immediately on startup.
  const preSpawnRecord = { ...job, status: "queued", phase: "queued", pid: null, logFile, request };
...
async function handleTask(argv) {
  ...
  if (options.background) {
    ...
    const request = { cwd, model, prompt, write, resumeLast, jobId: job.id };
  ...
  await runForegroundCommand(
    job,
    (progress) => executeTaskRun({ cwd, model, prompt, write, resumeLast, jobId: job.id, onProgress: progress }),
    { json: options.json }
  );
}
...
async function handleCancel(argv) {
  ...
  const interrupt = await interruptAgyTurn();
