...
PR本文は **2000文字未満**（`wc -c` のバイト数ではなく、実際の文字数）に収める。`wc -m` はロケールによって正しく数えられないことがあるため、Pythonで確認する。

```bash
python -c "print(len(open('<一時ファイル>', encoding='utf-8').read()))"
```

2000文字以上の場合は、以下の優先順位で圧縮する。**「動作確認」の「確認済み」「未確認」の区別と正直さは、圧縮の際も絶対に削らない。**

1. 「補足」「レビューしていただきたい設計判断」など任意セクションを簡潔にする、または特に無ければセクションごと削除する。
2. 「変更ファイル」表は主要なファイルのみに絞り、説明も1行に短縮する。
3. 「実行方法・確認手順」のコマンド例は代表的なものだけ残す。
4. 「概要」「動作確認」の文章を簡潔化する（内容は削らず、表現を短くする）。

圧縮後、再度文字数を確認し、2000文字未満になるまで繰り返す。
...
- 本文は2000文字未満に収める。超える場合はステップ4の優先順位に従って圧縮し、`gh pr create` 実行前に必ずPythonで文字数を確認する（`wc -c`/`wc -m` はバイト数やロケール依存で不正確なため使わない）。
